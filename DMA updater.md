update_dma <- function() {
  ...
  dma <- list()
  dma$document <- read_excel(path = vec_ruw, sheet = 1, skip = 1, col_names = F)           # sheet 1
  ...
  dma$channel <- read_excel(path = vec_ruw, sheet = 3, skip = 2, col_types = c(rep("text", 14)))                           # sheet 2
  dma$channel <- dma$channel[,3:ncol(dma$channel)]
    ...
    output_naam <- gsub(" \\(Ruw\\)", "", basename(vec_ruw), ignore.case = T)
